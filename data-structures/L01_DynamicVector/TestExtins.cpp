...
void testCreeazaMultime() {
	Multime m;
	assert(m.dim() == 0);
	assert(m.vida() == true);
}

void testAdaugaMultime() {
	Multime m;
	...
	assert(m.dim() == 3);
	assert(m.adauga(5) == false);
	assert(m.dim() == 3);
	assert(m.cauta(1) == true);
	assert(m.cauta(2) == false);
}

void testStergeMultime() {
	Multime m;
	for (int i = 0; i < 10; i++) {
		m.adauga(i);
	}
...
void testExceptiiMultime() {
	Multime m;
	assert(m.vida() == true);
	assert(m.dim() == 0);
	assert(m.sterge(10) == false);
	assert(m.cauta(10) == false);
	assert(m.adauga(10) == true);
