...
private:
  ...
  int inf = 9999;
  int start;
  Matrix MatriceAdiacenta, MatriceIncidenta, MatriceDistante, ListaAdiacenta;
  Lista edgesList;
...
public:
  ...
  void Labirint(string filepath) {
    ...
    for (int j = 3; j < m - 3; j++)
      if (M[0][j] == 0 && M[0][j - 1] == 0 && M[0][j + 1] == 0 &&
          M[0][j - 2] == 1 && M[0][j + 2] == 1) {
        start.x = 0;
        start.y = j;
        break;
    ...
    for (int j = 3; j < m - 3; j++)
      if (M[n - 1][j] == 0 && M[n - 1][j - 1] == 0 && M[n - 1][j + 1] == 0 &&
          M[n - 1][j - 2] == 1 && M[n - 1][j + 2] == 1) {
        finish.x = n - 1;
        finish.y = j;
        break;
      }
